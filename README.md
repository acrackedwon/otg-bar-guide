...
- **콘텐츠: 레포 안의 마크다운 파일** (`content/*.md`) — 외부 서비스 의존 없음. 사장님이 폰 브라우저에서 GitHub로 고치면 1~2분 뒤 자동 반영됩니다.
...
```
content/*.md        콘텐츠. 파일 하나 = 항목 하나. 여기만 고치면 됩니다
index.html          진입점
...
lib/markdown.ts     content/*.md → 사이트 데이터. 콘텐츠 출처 교체 지점
lib/content.ts      타입 정의, 영상 링크 파싱
```
...
- **콘텐츠 출처** — 글이 많아져 GitHub 편집이 번거로워지면 노션 연동이나 사이트 내 업로드로 교체. 고칠 곳은 `lib/markdown.ts`의 `loadContent()` 하나이고, 화면과 타입은 그대로입니다.
